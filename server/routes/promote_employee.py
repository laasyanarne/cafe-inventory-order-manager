...
def get_employees(current_user_id):
    ...
    # Join user_account with employee to get all info
    cursor.execute("""
        SELECT 
            ua.EID as id, 
            ua.email, 
...
def promote_employee(current_user_id, employee_id):
    ...
    cursor.execute("SELECT EID FROM `user_account` WHERE EID = %s", (employee_id,))
    ...
    cursor.execute("UPDATE `user_account` SET Access_level = 'manager' WHERE EID = %s", (employee_id,))
    conn.commit()
    cursor.close()
    conn.close()
